...
const dogsMetadataUrl =
	"https://ipfs.io/ipfs/Qma1wY9HLfdWbRr1tDPpVCfbtPPvjnai1rEukuqSxk6PWb?filename=undefined";
const techEventMetadataUrl =
	"https://ipfs.io/ipfs/QmchRqWmRiHP2uXBGxT7sJJUJChDddHpyApoH94S3VkH42?filename=undefined";
const yellowCrownMetadataUrl =
	"https://ipfs.io/ipfs/QmVXBCJcDtgtZfx77W86iG5hrJnFjWz1HV7naHAJMArqNT?filename=undefined";
const ashleyMetadataUrl =
	"https://ipfs.io/ipfs/QmdiA6eywkjMAVGTYRXerSQozLEBA3QpKmAt1E1mKVovhz?filename=undefined";
...
async function setupMarket(marketplaceAddress, nftAddress) {
	...
	marketplaceAddress =
		marketplaceAddress ||
		process.env[`MARKETPLACE_CONTRACT_ADDRESS_${networkName}`];
	...
	await marketplaceContract.cancelMarketItem(
		nftContractAddress,
		codeconMarketItemId
	);
	console.log(
		`${acc1.address} canceled market item for token ${codeconTokenId}`
	);

	const yellowMintTx = await nftContract
		.connect(acc2)
		.mintToken(yellowCrownMetadataUrl);
	const yellowTokenId = await getMintedTokenId(yellowMintTx);
	const ashleyMintTx = await nftContract
		.connect(acc2)
		.mintToken(ashleyMetadataUrl);
	const ashleyTokenId = await getMintedTokenId(ashleyMintTx);
	await marketplaceContract
		.connect(acc2)
		.createMarketItem(nftContractAddress, yellowTokenId, price, {
			value: listingFee,
		});
	await marketplaceContract
		.connect(acc2)
		.createMarketItem(nftContractAddress, ashleyTokenId, price, {
			value: listingFee,
		});
	console.log(
		`${acc2.address} minted tokens ${yellowTokenId} and ${ashleyTokenId} and listed them as market items`
	);
	...
	await marketplaceContract
		.connect(acc2)
		.createMarketSale(nftContractAddress, dogsTokenId, { value: price });
	await nftContract
		.connect(acc2)
		.approve(marketplaceContract.address, dogsTokenId);
	await marketplaceContract
		.connect(acc2)
		.createMarketItem(nftContractAddress, dogsTokenId, price, {
			value: listingFee,
		});
	console.log(
		`${acc2.address} bought token ${dogsTokenId} and put it for sale`
	);

	await marketplaceContract
		.connect(acc2)
		.createMarketSale(nftContractAddress, webArTokenId, { value: price });
